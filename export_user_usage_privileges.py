...
file_name = 'user_usage_privileges.xlsx'

file = output_dir + file_name

book = xlsxwriter.Workbook(file)
sheet = book.add_worksheet('user_usage_privileges')
...
with session_scope() as db:
    for user_id in range(1, max_user_id + 1, 1):
        ...
        if trace_rows.rowcount == 0 and usage_rows.rowcount == 0:
            continue
...
#            sheet.write(cur_row, col, 0)
        ...
        for row in trace_rows:
            ...
            user_id_time_result = markers.get(time)
            if user_id_time_result is None:
                user_id_time_result = 0

            user_id_time_result += privilege_level
            markers[time] = user_id_time_result

        for time, value in markers.items():
            sheet.write(cur_row, date_col_dict[time], value)
